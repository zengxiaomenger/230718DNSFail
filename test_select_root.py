...
file_name='other_data/root_nameserver.txt'
Lis_root_nameserver=[]
fin=open(file_name,'r',encoding='utf-8')
for line in fin:
    Lis_root_nameserver.append(line.strip())
    print(line.strip())
...
for line in tqdm(csv_in):
    ...
    if QorR=='0':
        continue
    if Resolver in Lis_root_nameserver:
...
dfout_sorted.to_csv('other_data/test_select_root.csv')